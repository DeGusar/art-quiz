...
import { timerProgress, countTimer } from "./questionsAuthorsPage";
...
function audioChangeVolume() {
  ...
  volumeScale.style.background = `linear-gradient(to right, rgb(3, 168, 3) 0%, rgb(3, 168, 3) ${
    volume * 100
  }%, #A4A4A4 ${volume * 100}%, #A4A4A4 100%)`;
}
...
function audioMute() {
  ...
  }%, #A4A4A4 ${volumeScale.value * 100}%, #A4A4A4 100%)`;
}
